...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Single-pass economic + ranking audit for frozen V2.1 cross-market+regime predictions"
    )
    p.add_argument("csv")
...
def main(argv: list[str] | None = None) -> int:
    ...
    base = V2Config()
    print(f"Building V2.1 predictor | {args.symbol} | peers={','.join(sorted(peers))}", flush=True)
    ...
    print(f"Scored={len(rank_rows)} blocked_feature={blocked_feature} blocked_event={blocked_event}", flush=True)
    ...
    if args.ranking_output_json:
        ...
        payload = {
            ...
            "note": "Single-pass diagnostic using the same frozen V2.1 predictions as the economic audit.",
        }
        output.write_text(json.dumps(payload, indent=2, allow_nan=False), encoding="utf-8")
