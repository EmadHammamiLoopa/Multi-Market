...
class V21CrossMarketRegimePredictor:
    """Frozen V2 policy with causal regime and masked cross-market context."""
    ...
    def __init__(
        ...
        *,
        ...
        self.feature_points, self.clean_feature_indices = build_v21_feature_points(
            bars,
    ...
    def feature_names(self) -> tuple[str, ...]:
        return v21_feature_names(self.peers)
    ...
    def _train_for_index(self, decision_index: int) -> None:
        ...
        if len(eligible) < cfg.min_train_rows:
            raise ValueError(
                f"V2.1 needs at least {cfg.min_train_rows} labeled historical rows; only {len(eligible)} are available at this timestamp"
            )
    ...
    def predict_at_index(self, decision_index: int) -> V2Decision:
        ...
        if point is None:
            raise ValueError("decision index does not have a clean contiguous V2.1 feature history")
