...
def _sse_and_sst(metrics: dict[str, object]) -> tuple[float, float]:
    ...
    denominator = 1.0 - r2
    if denominator <= 0.0:
        raise ValueError("cannot reconstruct positive SST from reported metrics")
    return sse, sse / denominator
...
def _pooled_incremental(
    ...
    *,
    ...
    for payload in payloads:
        block = payload["pooled"][model_name][PRIMARY_HORIZON]
        base = block["base"][key]
        cross = block["cross"][key]
        ...
        if abs(base_sst - cross_sst) > max(1e-8, 1e-8 * abs(base_sst)):
            raise AssertionError("BASE and CROSS must share the same realized target variance")
        base_sse += base_piece
        cross_sse += cross_piece
        total_sst += base_sst
    if total_sst <= 0.0:
        raise ValueError("no pooled variance available")
...
def summarize(payloads: list[dict[str, object]]) -> dict[str, object]:
    ...
    for payload in payloads:
        increments = [
            float(payload["pooled"][model][PRIMARY_HORIZON]["incremental_r2"])
            for model in MODELS
        ]
    ...
    conditions = {
        "pooled_positive_both_models": all(pooled[model] > 0.0 for model in MODELS),
        ...
        "non_jump_nonnegative_both_models": all(
            pooled_non_jump[model] >= 0.0 for model in MODELS
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Apply the frozen V2.3 Phase 0 promotion rule")
...
def main(argv: list[str] | None = None) -> int:
    ...
    for model, value in summary["pooled_incremental_r2"].items():
        print(f"pooled_6bar_{model}_incremental_r2={value:+.8f}")
    for model, value in summary["pooled_non_jump_incremental_r2"].items():
        print(f"pooled_non_jump_6bar_{model}_incremental_r2={value:+.8f}")
